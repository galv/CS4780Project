    ...
    def __init__(self, source = None, camera = None, colorSpace = ColorSpace.UNKNOWN,verbose=True, sample=False, cv2image=False, webp=False):
        ...
        **NOTES**
        ...
        if (type(source) == cv.cvmat):
            ...
            if False: #we have a 3 channel array
                ...
                source = source.astype(np.uint8) #Should cause no change
                old_source = source
                ### TODO: Convert to bgr, bgr, bgr, instead of all red then all green then all blue.
                self._bitmap = cv.CreateImage((32, 32), cv.IPL_DEPTH_8U, 3)
                channel = cv.CreateImageHeader((32, 32), cv.IPL_DEPTH_8U, 1)
                #initialize an empty channel bitmap
                cv.SetData(channel, source.tostring(),
                    source.dtype.itemsize * 32)
